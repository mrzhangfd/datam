...
@SpringBootTest
public class DatamApplicationTests {
    ...
    private List<String> list = Arrays.asList("丁汝昌738INFOB=5", "丘吉尔Z9R992MG=7", "东乡平八郎72JR4LO6=3", "严凤英NVYAIRA7=3",
            ...
            "麦克阿瑟PJG9BR81=3", "麦加CFKHLCJI=3", "麦卡锡71W1018T=4", "麦哲伦IQT0PBM9=4", "黄兴WJP5U4RC=5", "黄炎培Q6FG66L2=3", "黄飞鸿N5GJB2LA=3", "黑格尔JDLDCNGG=3");


    @Test
    public void getPersonNum() {
        int num = mongoDao.getAllPerson().size();
    ...
    public void test() {
        ...
        for (Doc doc : allDoc) {
            ...
            /*if ("-".equals(doc.getDate().substring(0, 1))) {
                System.out.println("==========================================");
                System.out.println(doc.getDate().substring(0, 1));
    ...
    public void testFind() throws IOException {
        ...
        HashSet<String> objects = mongoDao.getDocByNames(nameList);
    ...
    public void buildNewEvent() {
        list = new ArrayList<>(Arrays.asList("马丁·路德·金5537GF3K=5", "马克·吐温0IX7QIQY=4", "马兰·梅森IM15W09S=3"));
        ...
        List<Doc> docs=mongoDao.getDocByName("朱德");
        Person person=mongoDao.getPersonByName("朱德");

        List<NewEvent> newEvents=new ArrayList();
        for(Doc doc:docs){
            NewEvent newEvent=new NewEvent();
            newEvent.set_id(doc.get_id());
            newEvent.setObj_id("");
            newEvent.setDate(doc.getDate());
            newEvent.setEventIntro("");
            newEvent.setEventDetail(doc.getEvent());
            System.out.println(doc.getPName());
            //newEvent.setPName(doc.getPName());
            newEvent.setSName(doc.getSName());
            newEvent.setObj_id(person.get_id());
            newEvents.add(newEvent);
            //System.out.println(newEvent.getPName());

        }
        //System.out.println(newEvents);
        //System.out.println(newEvents.get(0).getPName());
    }
    @Test
    public void buildNewPerson() {
        list = new ArrayList<>(Arrays.asList("马丁·路德·金5537GF3K=5", "马克·吐温0IX7QIQY=4", "马兰·梅森IM15W09S=3", "马克·吐温0IX7QIQY=4"));
    ...
    public void buildEvent(){
        String tableName= HbaseModelUtil.EVENTS_TABLE;
        List<Result> results= hbaseDao.findAll(tableName);
        ...
        for (Result result : results) {
            ...
            for (KeyValue kv : result.list()) {
                ...
                realEntity = null;//objectServiceImpl.packageModel(realEntity, hbaseModel);
            }
